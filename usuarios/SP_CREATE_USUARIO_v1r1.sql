	...
	ELSE
		...
		IF vItems > 1 THEN
			...
			IF FN_CHECKTOKEN(data_token, data_usuarioid) = 1 THEN
				...
				INSERT INTO usuarios_tb(usuario, nombre, email, telefono, imagen, tipo, rolid, iniciales)
				VALUES (data_usuario, data_nombre, data_email, data_telefono, data_imagen, data_tipo, data_rolid, data_iniciales);
				-- Guardar en la bitácora.
				INSERT INTO logs_tb (usuarioid, direccionip, plataforma, programa, sentencia, token) VALUES (data_usuarioid, data_direccionip, 1, "SP_CREATE_USUARIO", vSentencia, data_token);
				SELECT "ok";
				SET rowCount = 1;
